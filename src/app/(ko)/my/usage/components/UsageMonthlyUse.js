 ...
 */
UsageMonthlyUse.propTypes = {
	activeCard: PropTypes.string,
};
export default function UsageMonthlyUse({activeNo}) {
					...
					<>
						<div className={'brief-wrap'}>
							<p>{`${fncMaskComma(data.totalAmt)}원`}</p>
							{
								data.diffAmt < 0 ? (
									<p>
										지난달 보다 <span className={'decrease'}>{fncMaskComma(data.diffAmt)}원 덜</span> 썼어요.
									</p>
								) : (
									<p>
										지난달 보다 <span className={'increase'}>{fncMaskComma(data.diffAmt)}원 더</span> 썼어요.
									</p>
								)
							}
						</div>
