		...
		<>
			...
			>
				<div className={`inner-header ${stateTransparent ? 'border-0' : 'border-b'}`}>
					...
					<nav className={'gnb-wrap'} aria-label={'사이트 주요 메뉴'}>
						{
							GnbMap.map((gnb) => {
								const isOpen = hoveredId === gnb.id;
								return (
									<div key={gnb.id} className={'h-full'} role={'none'} onMouseLeave={() => setHoveredId(null)}>
										<button
											key={gnb.id}
											className={`
												h-full px-5 py-6
										...
										>
											<p className={`text-body-2xl font-semibold ${colorGnbTextByState(isOpen)}`}>
												{gnb.name}
											</p>
										</button>
										<GnbDropdown
				...
				data.isSitemap && (
					...
					>
						<div
							...
							aria-label={'데스크탑 전체 메뉴'}
							role={'dialog'}
							aria-labelledby={'sitemap-title'}
						>
							<div className={'sitemap-inner-wrap'}>
								<div className={'sitemap-title-wrap'}>
									<p className={'text-dynamic-icon-brand-primary'}>
										<span
											id={'sitemap-title'}
											className={'text-heading-sm font-semibold mr-16'}>
											전체메뉴
										</span>
										<span
											className={'font-prosto'}
											aria-hidden={true}
										>
											SITEMAP
										</span>
								...
								<div>
									{
										GnbMap.map((gnb) => (
											<div key={gnb.id} className={'sitemap-group-wrap'}>
												<div className={'sitemap-gnb-wrap'} aria-label={`메뉴 대분류: ${gnb.name}`}>
													{gnb.name}
												</div>
												<div className={'sitemap-lnb-wrap grid-cols-2 wide:grid-cols-4'}
												     aria-label={'2차 메뉴(소분류)'}>
													{
														gnb.lnb?.map((lnb) => (
															<button
																key={lnb.id}
					...
					['main'].includes(headerType) && (
						...
						<>
							<button
						        ...
						        onClick={() => fncCallbackEvent('goHome')}
								onKeyDown={(e) => {
									if(e.key === 'Enter' || e.key === ' ') fncCallbackEvent('goHome')
								}}
						        onFocus={() => fncCallbackEvent('callListenerCloseLanguageChanger')}
						        onMouseEnter={() => fncCallbackEvent('callListenerCloseLanguageChanger')}
							>
					...
					['back'].includes(headerType) && (
						...
						<>
							<button
								...
								onKeyDown={(e) => {
									if(e.key === 'Enter' || e.key === ' ') fncCallbackEvent('goBack')
								}}
							>
