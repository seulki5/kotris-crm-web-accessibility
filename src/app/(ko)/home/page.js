...
export function DtHome({data, fncCallbackEvent}) {
	...
	const renderHeaderComponent = (item, index) => {
		...
		const panelId = HOME_FAQ_PANEL_ID(item.faqId) + 'h';
		return (
			<button
				key={item.faqId}
				type={'button'}
	...
	const renderBodyComponent = (item) => {
		...
		const panelId = HOME_FAQ_PANEL_ID(item.faqId) + 'b';
		return (
			<div
				id={panelId}
				role={'region'}
	...
	return (
		<main id={'home'} className={'w-full -mt-[90px] overflow-hidden'} aria-labelledby={'page-name-dt'}>
			...
			<fieldset role={'group'} className={'card-wrap'}>
			...
			<fieldset role={'group'} className={'service-wrap'}>
				<legend className={'sr-only'}>
					레일플러스 대표 서비스 안내
				</legend>
			...
			<fieldset role={'group'} className={'guide-wrap'}>
				<legend className={'sr-only'}>
					레일플러스 이용가이드
				</legend>
				<p className={'sub-catchphrase'}>
			...
			</fieldset>
			{
				data.bannerList?.length > 0 && (
					<fieldset role={'group'} className={'banner-wrap'}>
						<legend className={'sr-only'}>
							배너
						</legend>
						<Swiper
			...
			<fieldset role={'group'} className={'store-wrap'}>
				<legend className={'sr-only'}>
					레일플러스 어플 다운로드 스토어 안내
				</legend>
...
export function MoHome({data, fncCallbackEvent}) {
	...
	return (
		<main id={'home'} className={'w-full -mt-[90px] overflow-hidden'} aria-labelledby={'page-name-mo'} style={{padding: 0}}>
			...
			<fieldset role={'group'} className={'store-wrap'} ref={appStoreDivRef}>
				<legend className={'sr-only'}>
					레일플러스 어플 다운로드 스토어 안내
				</legend>
				<Image
