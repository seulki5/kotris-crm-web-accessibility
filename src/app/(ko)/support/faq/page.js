...
// const
const SUPPORT_FAQ_TRIGGER_ID = (faqId) => `support-faq-trigger-${faqId}`;
const SUPPORT_FAQ_PANEL_ID = (faqId) => `support-faq-panel-${faqId}`;
...
export function DtSupportNoticeList({data, fncCallbackEvent, ref}) {
	...
	const renderHeaderComponent = (item, index) => {
		...
		const triggerId = SUPPORT_FAQ_TRIGGER_ID(item.faqId);
		const panelId = SUPPORT_FAQ_PANEL_ID(item.faqId) + 'h';
		return (
			<button
				key={item.faqId}
				id={triggerId}
	...
	const renderBodyComponent = (item) => {
		...
		const panelId = SUPPORT_FAQ_PANEL_ID(item.faqId) + 'b';
		return (
			<div
				id={panelId}
				role={'region'}
				aria-labelledby={SUPPORT_FAQ_TRIGGER_ID(item.faqId)}
									...
									data.faqList.list.map((item, index) => {
										return (
											<Accordion
												...
												panelId={accordionA11yIds('support-faq', item.faqId).panelId}
