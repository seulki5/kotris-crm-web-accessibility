...
import CardRefundStep1 from '@/app/(ko)/my/claim/components/CardRefundStep1';
import CardRefundStep2 from '@/app/(ko)/my/claim/components/CardRefundStep2';
import CardRefundStep3 from '@/app/(ko)/my/claim/components/CardRefundStep3';
 ...
 */
export default function LayoutRefund({onChangeTab}) {
		...
		if (rules) {
			...
			if (Object.keys(res).length === 0) {
				if ([CODE.CARD_SEG_MO_PRE, CODE.CARD_SEG_MO_POST].includes(selectedCard?.mypgCardSeCd)) {
					await fncCallbackEvent('doRefund')
					// setStep(step + 2);
				} else {
					setStep(step + 1);
					fncScrollToTop();
