...
/**
 * @description: Rail+ 충전내역 통계 화면 입니다.
 * @screenID:    -
 * @screenPath:  홈 > 마이페이지 > Rail+ 충전내역 통계
 * @author       $Author$
 * @since        $Date$
 * @version      $Revision$
 * Copyright (C) 2025 by STraffic co.,Ltd. All right reserved.
 */
UsageMonthlyCharge.propTypes = {
	activeCard: PropTypes.string,
};
export default function UsageMonthlyCharge({activeNo}) {
 ...
 * @screenID:    -
 */
DtUsageMonthlyCharge.propTypes = {
	data: PropTypes.object,
	fncCallbackEvent: PropTypes.func
};
export function DtUsageMonthlyCharge({data, fncCallbackEvent}) {
	const thisMonth = moment().format('YYYYMM');
				...
				data.statsData?.length ? (
					<>
						<div className={'brief-wrap'}>
							...
							<p>
								이번달은 <span className={'increase'}>{fncMaskComma(data.totalAmt)}원</span> 충전되었습니다.
							</p>
						</div>
						<div className={'statistics-wrap'}>
							...
							<ul className="legend-wrap" aria-label={'충전내역통계 항목별 비율 목록'}>
								{
					...
					<div className={'empty-wrap py-50'}>
						<EmptyList />
						<p>이용내역이 없습니다.</p>
					</div>
				)
 ...
 * @screenID:    -
 */
MoUsageMonthlyCharge.propTypes = {
	data: PropTypes.object,
	fncCallbackEvent: PropTypes.func
};
export function MoUsageMonthlyCharge({data, fncCallbackEvent}) {
	const thisMonth = moment().format('YYYYMM');
	const blockNextMonth = thisMonth === data.dlngYm;
